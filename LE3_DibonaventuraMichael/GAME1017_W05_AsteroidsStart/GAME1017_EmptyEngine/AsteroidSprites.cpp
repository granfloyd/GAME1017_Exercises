...
void ShipAsteroids::Update()
{
	...
	case STATE_IDLING:
		...
		break;
	case STATE_MOVING:
		//update vector dletas from angle
		m_dx = cos(MAMA::Deg2Rad(m_angle - 90));//param is angle in radians
		m_dy = sin(MAMA::Deg2Rad(m_angle - 90));
		// Update velocities.
		m_velX += m_dx * m_thrust;
		m_velY += m_dy * m_thrust;
		// Clamp velocities. min(max())
		m_velX = max(m_velX, -(fabs(m_dx) * m_velMax)), fabs(m_dx)* m_velMax;
		m_velY = max(m_velY, -(fabs(m_dy) * m_velMax)), fabs(m_dy)* m_velMax;
